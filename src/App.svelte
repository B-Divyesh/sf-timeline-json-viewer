<script lang="ts">
  ...
  let updateWaiting: ServiceWorker | null = null, reloadForUpdate = false, routeAnnouncement = '', scope: StorageScope = 'real';
  ...
  $: description = isNotFound ? 'The requested Field Atlas page does not exist.' : demoMode ? 'Explore a sample Google Timeline in Field Atlas.' : 'Browse an exported Google Timeline privately in your browser.';

  onMount(() => {
    void initialize(); void checkConnection();
  ...
  async function initialize() {
    if (isLegal || isNotFound) { restored = true; return; }
    selectedDate = localStorage.getItem(dateKey) ?? '';
    try { dataset = await loadDataset(scope); if (demoMode && !dataset) { dataset = structuredClone(demoDataset); await saveDataset(dataset, 'demo'); } hydrateDates(); }
    catch { error = 'The saved timeline could not be opened. Import the Timeline JSON again.'; }
    restored = true;
  ...
  async function importFile(file: File) {
    ...
    importing = true; progress = 2; progressMessage = 'Opening a local worker…';
    await new Promise<void>((resolve) => {
      const worker = new Worker(new URL('./lib/parser.worker.ts', import.meta.url), { type: 'module' }), finish = () => { importing = false; worker.terminate(); resolve(); };
      worker.onmessage = async (event: MessageEvent<WorkerResponse>) => { const message = event.data; if (message.type === 'progress') { progress = message.progress; progressMessage = message.message; } if (message.type === 'error') { error = message.message; finish(); } if (message.type === 'complete') { try { await saveDataset(message.dataset, scope); dataset = message.dataset; hydrateDates(); progress = 100; progressMessage = `${dataset.events.length.toLocaleString()} entries saved in this browser.`; } catch { error = 'The file parsed, but this browser could not save it.'; } finish(); } };
      worker.onerror = () => { error = 'The background parser stopped. Close other tabs and import again.'; finish(); }; worker.postMessage({ file });
    });
  }
  ...
  async function removeTimeline() { if (!confirm(`Remove the saved timeline “${dataset?.name}”? Your source file is unchanged.`)) return; await clearDataset(scope); dataset = undefined; selectedDate = ''; error = ''; }
</script>

<svelte:head>
  <title>{title}</title><meta name="description" content={description} /><link rel="canonical" href={`https://timeline-json-viewer.sociobot.in${path}`} />
...
{:else}
  <main id="main" class:has-data={dataset} on:dragover={(event) => { event.preventDefault(); dragging = true; }} on:dragleave={() => dragging = false} on:drop={drop}>
    ...
    {:else if !dataset}
      <section class="welcome" aria-labelledby="welcome-title"><div class="welcome-copy"><p class="kicker">Local Timeline JSON viewer</p><h1 id="welcome-title" tabindex="-1">Browse your exported Google Timeline</h1><p>For people with a Timeline JSON file they cannot open.</p><div class="welcome-actions"><button class="primary-button" type="button" on:click={startDemo}>Try it with sample data</button><label class="primary-file secondary-file"><input type="file" accept=".json,application/json" on:change={chooseFile} /><span>Open Timeline JSON</span></label></div><p class="drop-hint">The sample opens the calendar, entries, map, and exports.</p><ul class="trust-list"><li><span aria-hidden="true">01</span> Runs in your browser</li><li><span aria-hidden="true">02</span> Sample works after the first visit</li><li><span aria-hidden="true">03</span> Export CSV, GPX, or KML</li></ul></div><div class="atlas-key" aria-label="Timeline entry types"><p class="sheet-label">Timeline entry types</p><div><i class="mark visit"></i><span>Visits and named places</span></div><div><i class="mark route"></i><span>Trips and paths</span></div><div><i class="mark point"></i><span>Raw location records</span></div><p class="coordinate">PARSED IN THIS BROWSER</p></div></section>
      <section class="landing-section preview-section"><h2>See a sample timeline first</h2><p>The sample opens realistic visits, a walking route, and a cycling route. It never reads your saved timeline.</p><button class="secondary-button" type="button" on:click={startDemo}>Open the sample viewer</button></section>
      <section class="landing-section"><h2>How it works</h2><ol class="steps"><li><strong>Open a file</strong><span>Choose Timeline.json, Records.json, or a legacy Google Takeout file.</span></li><li><strong>Browse days</strong><span>Find visits, trips, and raw records in the calendar and coordinate map.</span></li><li><strong>Take a copy</strong><span>Export a selected date range as CSV, GPX, or KML.</span></li></ol></section>
